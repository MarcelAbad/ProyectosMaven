...
public class LibroService {
    ...
    private void validarLibro(Libro libro) {
        ...
        if (libro.getPrecio() <= 0) {
            throw new LibroException("El precio no puede ser menor que 0");
        }
        if (libro.getStock() <= 0) {
        ...
        boolean existe = libros.stream()
                .anyMatch(libro1 -> libro1.getTitulo().equals(libro.getTitulo()));

        if (existe) {
            throw new LibroException("El libro ya existe");
        }
    }
    ...
    public void actualizarPrecio(Libro libro, double nuevoPrecio) {
        if (nuevoPrecio <= 0) {
            throw new LibroException("El precio no puede ser menor que 0");
        }
    ...
    public void actualizarStock(Libro libro, int nuevoStock) {
        if (nuevoStock <= 0) {
            throw new LibroException("El stock no puede ser menor que 0");
        }
